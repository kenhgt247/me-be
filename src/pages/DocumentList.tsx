...
export const DocumentList: React.FC = () => {
  ...
  // ✅ KHÔI PHỤC BIẾN filteredDocs VÀ visibleCount ĐỂ FIX LỖI WHITE SCREEN
  const filteredDocs = useMemo(() => {
    if (!searchTerm) return docs;
    return docs.filter(d => d.title.toLowerCase().includes(searchTerm.toLowerCase()));
  }, [docs, searchTerm]);

  // Trong logic phân trang server, biến visibleDocs chính là filteredDocs (vì ta lấy từ server về)
