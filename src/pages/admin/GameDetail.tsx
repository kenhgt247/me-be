...
export const GameDetail: React.FC = () => {
  ...
  const handleSaveLevel = async () => {
    if (!gameId) return;
    if (
      !formLevel.instruction.text &&
      !formLevel.instruction.imageUrl &&
      !formLevel.instruction.audioUrl
    ) {
    ...
    try {
      if (game.gameType === 'story') {
        ...
        const rawData = await generateGameContent(
          ...
          aiCount,
          'emoji',
          game.category, // category thực tế từ Firestore
          aiLanguage,
          aiLearningGoal,
          aiExtra
        );

        const safeArr: any[] = Array.isArray(rawData) ? rawData : [];

        // ✅ FLASHCARD: không dùng q/opts/a kiểu trắc nghiệm
        if (game.gameType === 'flashcard') {
          const convertedFlashcardLevels: GameLevel[] = safeArr.map((item: any, idx: number) => {
            const letter = (item?.letter ?? item?.front ?? item?.q ?? '').toString().trim();
            const word = (item?.word ?? item?.back ?? item?.a ?? '').toString().trim();
            const vi = (item?.vi ?? item?.meaning ?? '').toString().trim();

            let text = '';
            if (aiLanguage === 'Tiếng Anh') text = `${letter} – ${word}`.trim();
            else if (aiLanguage === 'Song ngữ') {
              text = vi ? `${letter} – ${word} (${vi})` : `${letter} – ${word}`;
            } else {
              // Tiếng Việt: vẫn hiển thị chữ + nghĩa, tránh engine “trống”
            ...
            return {
              ...
              correctAnswerId: '', // flashcard không cần đáp án
              order: levels.length + idx + 1
            };
          });

          setGeneratedLevels(convertedFlashcardLevels);
          return;
        }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
        <div className="flex items-center gap-4">
          <button
            ...
            className="p-3 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors text-gray-600"
          >
            <ArrowLeft size={22} />
          </button>
          <div>
            <div className="flex items-center gap-3">
              <span
                className={`w-12 h-12 flex items-center justify-center rounded-xl text-2xl ${game.color} text-white shadow-sm`}
              >
                {game.icon}
              </span>
            ...
            <div className="flex gap-2 mt-2">
              ...
              <span className="bg-blue-50 text-blue-600 px-2 py-0.5 rounded text-xs font-bold uppercase">
                {categories.find((c) => c.id === game.category)?.label}
              </span>
              {game.gameType !== 'html5' && game.gameType !== 'story' && (
                <span className="text-gray-500 font-medium text-xs self-center">
                  ({levels.length} màn chơi)
                </span>
              )}
            </div>
          </div>
        ...
        <button
          onClick={() => {
            setShowAiModal(true);
            setAiTopic(game.title);
          }}
          className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-5 py-3 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-indigo-200 active:scale-95 transition-all"
        >
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200">
          ...
          <div className="space-y-4">
            ...
            <input
              ...
              className="w-full border p-4 rounded-xl outline-none focus:ring-4 focus:ring-indigo-100"
              placeholder="https://..."
            />
            <button
      ...
      {game.gameType === 'story' && (
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200">
          ...
          <textarea
            ...
            className="w-full border p-4 rounded-xl outline-none focus:ring-4 focus:ring-indigo-100 min-h-[300px]"
            placeholder="Ngày xửa ngày xưa..."
          />
          <button
            onClick={handleSaveConfig}
            disabled={savingSettings}
            className="mt-4 bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-700 flex items-center gap-2"
          >
        ...
        <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
              <div className="flex justify-between items-center mb-6 border-b pb-4">
                <h3 className="font-bold text-gray-800 flex items-center gap-2 text-lg">
                  {isEditingLevel ? '✏️ Chỉnh sửa Màn chơi' : '➕ Thêm Màn chơi mới'}
                </h3>
                {isEditingLevel && (
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 gap-2">
                  <input
                    value={formLevel.instruction.text || ''}
                    onChange={(e) =>
                      setFormLevel({
                        ...formLevel,
                        instruction: { ...formLevel.instruction, text: e.target.value }
                      })
                    }
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input
                        value={formLevel.instruction.imageUrl || ''}
                        onChange={(e) =>
                          setFormLevel({
                            ...formLevel,
                            instruction: { ...formLevel.instruction, imageUrl: e.target.value }
                          })
                        }
                        placeholder="URL Hình ảnh"
                        className="border p-3 pl-10 rounded-xl w-full text-sm"
                      />
                    </div>
                    <div className="relative flex-1">
                      <Volume2 size={18} className="absolute top-3.5 left-3 text-gray-400" />
                      <input
                        value={formLevel.instruction.audioUrl || ''}
                        onChange={(e) =>
                          setFormLevel({
                            ...formLevel,
                            instruction: { ...formLevel.instruction, audioUrl: e.target.value }
                          })
                        }
                  ...
                  {formLevel.items.map((item, idx) => (
                    <div
                      ...
                      className={`p-4 rounded-xl border-2 transition-colors ${
                        formLevel.correctAnswerId === item.id
                          ? 'border-green-500 bg-green-50/50'
                          : 'border-gray-100 bg-gray-50'
                      }`}
                    >
                      <div className="flex gap-3 mb-2">
                        <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center font-bold text-gray-500 shadow-sm">
                          {idx + 1}
                        </div>
                        <input
                          value={item.text || ''}
                          onChange={(e) => updateFormItem(idx, 'text', e.target.value)}
                          placeholder={`Lựa chọn ${idx + 1}`}
                          className="flex-1 border-b bg-transparent outline-none focus:border-blue-500 px-2"
                        />
                        <button
                      ...
                      <div className="grid grid-cols-2 gap-3 pl-11">
                        <input
                          ...
                          onChange={(e) => updateFormItem(idx, 'imageUrl' as any, e.target.value)}
                          placeholder="URL Hình ảnh"
                          className="bg-white border p-2 rounded text-xs"
                        />
                        <div className="flex items-center gap-2">
                          <input
                            type="radio"
                            checked={formLevel.correctAnswerId === item.id}
                            onChange={() =>
                              setFormLevel({ ...formLevel, correctAnswerId: item.id })
                            }
              ...
              <button
                ...
                className="w-full bg-gray-900 text-white py-4 rounded-xl font-bold hover:bg-black transition-all flex items-center justify-center gap-2 shadow-lg shadow-gray-300"
              >
          ...
          <div className="space-y-4">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 max-h-[80vh] overflow-y-auto">
              <h3 className="font-bold text-gray-800 mb-4 sticky top-0 bg-white pb-2 border-b z-10">
                Danh sách Màn chơi ({levels.length})
              </h3>
              <div className="space-y-2">
                {levels.length === 0 ? (
                  <p className="text-center text-gray-400 text-sm py-4">Chưa có màn chơi nào.</p>
                ) : (
                  levels.map((lvl, idx) => (
                    <div
                      key={lvl.id}
                      className="p-3 rounded-xl border border-gray-100 hover:border-blue-200 hover:bg-blue-50 transition-all group relative"
                    >
                      <div className="flex justify-between items-start">
                        <div className="flex gap-3">
                          <span className="bg-gray-200 text-gray-600 w-6 h-6 rounded flex items-center justify-center text-xs font-bold">
                            {idx + 1}
                          </span>
                          <div>
                            <p className="font-bold text-gray-800 text-sm line-clamp-1">
                              {lvl.instruction.text || '(Hình ảnh)'}
                            </p>
                        ...
                        <div className="flex gap-1">
                          <button
                            ...
                            className="p-1.5 hover:bg-white rounded text-blue-500"
                          >
                            <Code size={16} />
                          </button>
                          <button
              ...
              {generatedLevels.length > 0 ? (
                <div className="space-y-6">
                  <div className="flex justify-between items-center">
                    <p className="text-green-600 font-bold flex items-center gap-2 text-lg">
                      <Sparkles size={20} /> AI đã hoàn tất thiết kế:
                    </p>
                    <button
                      onClick={() => setGeneratedLevels([])}
                    ...
                    {generatedLevels.map((lvl, i) => (
                      <div key={i} className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <div className="text-xs text-gray-500">
                          {lvl.items.length} lựa chọn. Đáp án đúng ID:{' '}
                          {lvl.correctAnswerId?.substr(0, 5)}...
                        </div>
                      </div>
                    ))}
                  </div>
                  <button
                    onClick={saveGeneratedLevels}
                ...
                <div className="space-y-5">
                  <div className="flex items-center gap-2 p-3 bg-indigo-50 rounded-xl border border-indigo-100">
                    ...
                    <span className="text-xs font-bold text-indigo-700 uppercase">
                      Chuyên mục: {categories.find((c) => c.id === game.category)?.label}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="flex items-center gap-2 text-sm font-bold text-gray-700 mb-2">
                        <Bot size={16} /> Chủ đề chi tiết
                      </label>
                      <input
                        value={aiTopic}
                        onChange={(e) => setAiTopic(e.target.value)}
                        placeholder="Ví dụ: Bé học đếm số..."
                        className="w-full border p-4 rounded-xl outline-none focus:ring-4 focus:ring-indigo-100"
                      />
                    </div>
                    <div>
                      <label className="flex items-center gap-2 text-sm font-bold text-gray-700 mb-2">
                        <Languages size={16} /> Ngôn ngữ game
                      </label>
                      <select
                        value={aiLanguage}
                  ...
                  <div>
                    <label className="flex items-center gap-2 text-sm font-bold text-gray-700 mb-2">
                      <Target size={16} /> Mục tiêu học tập (Bắt buộc)
                    </label>
                    <textarea
                      value={aiLearningGoal}
                      onChange={(e) => setAiLearningGoal(e.target.value)}
                      rows={3}
                      placeholder="Ví dụ: Giúp bé nhận diện mặt số từ 1-10, học cách phát âm..."
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="md:col-span-3">
                      <label className="flex items-center gap-2 text-sm font-bold text-gray-700 mb-2">
                        <FileText size={16} /> Yêu cầu đặc biệt (Nếu có)
                      </label>
                      <input
                        value={aiExtra}
                        onChange={(e) => setAiExtra(e.target.value)}
                        placeholder="Ví dụ: Chỉ sử dụng emoji trái cây..."
                        className="w-full border p-4 rounded-xl outline-none focus:ring-4 focus:ring-indigo-100"
                      />
                    </div>
                  </div>
                  ...
                  {!aiLearningGoal.trim() && (
                    <p className="text-center text-xs text-orange-500 font-medium">
                      Mẹ vui lòng nhập Mục tiêu học tập để AI làm việc nhé!
                    </p>
                  )}
                </div>
              )}
