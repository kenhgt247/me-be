...
import {
  Ban, Search, ShieldCheck, Edit, X, Save, Loader2, ChevronDown, RefreshCw, UserPlus
} from 'lucide-react';
import {
  fetchUsersAdminPaginated,
  updateUserRole,
  updateUserInfo,
  createUserByAdmin, // ✅ thêm
} from '../../services/admin';
...
export const UserManagement: React.FC = () => {
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      await loadInitialUsers(); // refresh list
    } catch (e: any) {
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button
                          ...
                          className={`p-2 transition-all ${user.isAdmin ? 'text-purple-600' : 'text-gray-300'}`}
                          title="Toggle Admin"
                        >
                          <ShieldCheck size={16} />
                        </button>
                        ...
                        <button
                          onClick={() => {
                            if (confirm('Khóa người này?')) {
                              updateUserRole(user.id, { isBanned: !user.isBanned }).then(loadInitialUsers);
                            }
                          }}
                          className={`p-2 transition-all ${user.isBanned ? 'text-red-600' : 'text-gray-300'}`}
                          title="Ban/Unban"
                        >
                          <Ban size={16} />
                        </button>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-fade-in">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden animate-pop-in border border-gray-100">
            ...
            <div className="px-8 py-6 bg-gray-50/80 flex justify-end gap-3 border-t">
              <button onClick={() => setEditingUser(null)} className="px-6 py-2.5 font-black text-xs text-gray-400 uppercase">
                Hủy
              </button>
              <button
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-fade-in">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden animate-pop-in border border-gray-100">
            ...
            <div className="p-8 space-y-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Tên hiển thị (tuỳ chọn)</label>
                <input
                  type="text"
              ...
              <p className="text-xs text-gray-400 leading-relaxed">
                * Tạo user bằng server để <b>không làm admin bị đăng xuất</b>. Sau khi tạo, user sẽ xuất hiện trong danh sách.
              </p>
            </div>
