    ...
    <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl animate-pop-in flex flex-col max-h-[90vh]">
        <div className="p-4 border-b flex justify-between items-center">
          <h3 className="font-bold text-gray-800 flex items-center gap-2"><List size={20}/> Quản lý Danh mục</h3>
...
export const QuestionManagement: React.FC = () => {
  ...
  const loadInitialData = async () => {
    setLoading(true);
    const { questions: data, lastDoc: nextDoc, hasMore: more } = await fetchQuestionsAdminPaginated(null, 15);
    setQuestions(data); setLastDoc(nextDoc); setHasMore(more);
    setLoading(false);
  };
  ...
  const handleLoadMore = async () => {
    ...
    setLoadingMore(true);
    const { questions: newData, lastDoc: nextDoc, hasMore: more } = await fetchQuestionsAdminPaginated(lastDoc, 15);
    setQuestions(prev => [...prev, ...newData]); setLastDoc(nextDoc); setHasMore(more);
  ...
  const handleBulkAction = async (action: 'hide' | 'show' | 'delete') => {
    ...
    setSelectedIds(new Set()); loadInitialData();
  };

  return (
    <div className="space-y-6 pb-20">
      <div className="bg-white p-4 rounded-xl border shadow-sm flex flex-col md:flex-row justify-between gap-4">
        <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={18}/><input type="text" placeholder="Tìm câu hỏi..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-lg outline-none focus:ring-2 focus:ring-blue-100" /></div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-gray-50 text-xs uppercase text-gray-500 font-bold">
              <tr>
                <th className="px-6 py-4 w-10"><input type="checkbox" onChange={e => setSelectedIds(e.target.checked ? new Set(questions.map(q=>q.id)) : new Set())}/></th>
            ...
            <tbody className="divide-y text-sm">
              {loading ? (
                ...
                <tr key={q.id} className={`hover:bg-gray-50 ${selectedIds.has(q.id) ? 'bg-blue-50' : ''}`}>
                  ...
                  <td className="px-6 py-4"><p className="font-bold text-gray-900 line-clamp-1">{q.title}</p><span className="text-[10px] text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded uppercase">{q.category}</span></td>
                  <td className="px-6 py-4 text-gray-600">{q.author.name}</td>
                  <td className="px-6 py-4 text-center">{q.isHidden ? <span className="text-red-500 font-bold">Ẩn</span> : <span className="text-green-600 font-bold">Hiện</span>}</td>
                  <td className="px-6 py-4 text-gray-400">{new Date(q.createdAt).toLocaleDateString('vi-VN')}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-gray-900 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-6 z-50 animate-slide-up">
           <span className="font-bold text-sm">{selectedIds.size} mục đã chọn</span>
           <div className="flex gap-3">
              <button onClick={() => handleBulkAction('hide')} className="text-sm font-medium hover:text-blue-400 flex items-center gap-1"><EyeOff size={16}/> Ẩn</button>
              <button onClick={() => handleBulkAction('show')} className="text-sm font-medium hover:text-green-400 flex items-center gap-1"><Eye size={16}/> Hiện</button>
              <button onClick={() => handleBulkAction('delete')} className="text-sm font-bold text-red-400 flex items-center gap-1"><Trash2 size={16}/> Xóa</button>
           </div>
        </div>
      )}
